...
class run_GUI(GUI.GUI):
    ...
    def PortAssignment(self):
        ...
        logger.info('\tTEC port:{}'.format( self.TEC_PORT))
        logger.info('\tTechnosoft port:{}'.format(self.TECHNOSOFT_PORT ))
        logger.info('\tpump1:{}'.format( self.PUMP1_PORT))
        logger.info('\tGantry Vertical Axis ID:{}'.format( str(self.GANTRY_VER_AXIS_ID)))
    ...
    def p1_b_dispenseUntillbubble(self):
        ...
        while (input0>2.5):
            input0 = (self.labjack.getAIN(0))
            logger.info('----------------{}    position:'.format(input, self.pump1.get_plunger_position(1)))
            time.sleep(1)
        self.pump1.stop(1)
    ...
    def p1_b_pickupUntillbubble(self):
        logger.info("child: pickup until bubble")
